
const express = require('express');

const app = express();

const port = 3002;

const bodyParser = require("body-parser");
app.use(bodyParser.json());
app.use(bodyParser.urlencoded({ extended: true }));

app.listen(port, () => {
    console.log(`Example app listening on port ${port}`)
});

const COMMON = require('./COMMON');

const uri = COMMON.uri;

const mongoose = require('mongoose');

const carModel = require('./carModel');

const apiMobile = require('./api');
app.use('/api', apiMobile);

app.get('/', async (req, res) => {
    await mongoose.connect(uri);

    let cars = await carModel.find();

    console.log(cars);

    res.send(cars);

})

app.post('/add_xe', async (req, res) => {
    await mongoose.connect(uri);
    // let car = {
    //     ten: 'xe 3',
    //     namSX: 2024,
    //     hang: 'Mitsubishi',
    //     gia: 7500
    // }

    let car = req.body;

    console.log(car)

    let kq = await carModel.create(car);
    console.log(kq);

    let cars = await carModel.find();

    res.send(cars);

})

// app.delete('/xoa/:id', async (req, res) => {
//     await mongoose.connect(uri);

//     let id = req.params.id;
//     console.log(id);

//     // xu ly loi khi id khong dung
//     await carModel.deleteOne({_id: id});

//     res.redirect('../')
// }) 

// app.put('/update/:ten', async (req, res) => {
//     await mongoose.connect(uri);

//     console.log('Ket noi DB thanh cong');

//     let tenXe = req.params.ten;
//     console.log(tenXe);

//     // let tenXeMoi = tenXe + ' Phien ban moi 2024';

//     // await carModel.updateOne({ten: tenXe}, {ten: tenXeMoi});
//     await carModel.updateOne({ten: tenXe});

//     let xehois = await carModel.find({});

//     res.send(xehois);
// })
app.put('/update/:id', async (req, res) => {
    try {
        await mongoose.connect(uri);

        let id = req.params.id;
        let updatedCarData = req.body;

        let updatedCar = await carModel.findByIdAndUpdate(id, updatedCarData, { new: true });

        if (!updatedCar) {
            return res.status(404).json({ message: 'Xe không tồn tại' });
        }

        res.json(updatedCar);
    } catch (error) {
        console.error('Lỗi khi cập nhật xe:', error);
        res.status(500).json({ message: 'Có lỗi xảy ra khi cập nhật xe' });
    }
});
app.delete('/delete/:id', async (req, res) => {
    try {
        await mongoose.connect(uri);

        let id = req.params.id;

        // Tìm và xóa xe dựa trên ID
        let deletedCar = await carModel.findByIdAndDelete(id);

        if (!deletedCar) {
            // Nếu không tìm thấy xe với ID cung cấp, trả về một mã trạng thái 404 và một thông báo lỗi
            return res.status(404).json({ message: 'Không tìm thấy xe để xóa' });
        }

        // Nếu xóa thành công, chuyển hướng người dùng đến trang khác
        res.redirect('../');
    } catch (error) {
        // Nếu có bất kỳ lỗi nào xảy ra, trả về một mã trạng thái 500 và một thông báo lỗi
        console.error('Lỗi khi xóa xe:', error);
        res.status(500).json({ message: 'Có lỗi xảy ra khi xóa xe' });
    }
});


